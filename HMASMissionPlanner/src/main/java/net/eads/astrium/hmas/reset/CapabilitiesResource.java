...
public class CapabilitiesResource extends AbstractResource {
    ...
    private CapabilitiesDocument getRESETCapabilities(
        ...
        if (caps.getServiceIdentification().getProfileArray().length > 0) {
            caps.getServiceIdentification().removeProfile(0);
        }
        String profile = "http://www.opengis.net/reset/1.0";
        caps.getServiceIdentification().addProfile(profile);
        System.out.println("Profile set to : " + profile);
