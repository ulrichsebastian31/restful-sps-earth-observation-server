...
<ctl:package 
 ...
 xsi:schemaLocation="http://www.occamlab.com/ctl  ../apps/engine/resources/com/occamlab/te/schemas/ctl.xsd">	<!-- CTL FILE METADATA
  ...
  -->
	<ctl:test name="reset:ATS_14.1.2.6_GET_Sensor_Availability_Description_non_nominal_conditions">
		...
		<ctl:link title="14.1.2.6 Get Sensor Description/non_nominal_conditions">
		http://www.opengis.net/spec/RESET/1.0.0/conf/Core/GET_Sensor_Availability_Description-non-nominal
		</ctl:link>
		<ctl:code>
			<!-- #############################################################################################  -->
			<!-- #
          ...
          Pass if the assertion is satisfied; fail otherwise.
			...
			<xsl:variable name="endpoint.description" select="concat($sp.endpoint.HTTP.url, 'procedures/', 'NON_VALID_SENSOR', '/availibility')"/>
			...
			<xsl:if test="not ($response/*)">
				<ctl:message>ATS 14.1.2.6 validation failed</ctl:message>
				<ctl:fail/>
			</xsl:if>
		</ctl:code>
	</ctl:test>
