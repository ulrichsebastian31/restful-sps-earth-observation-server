<ctl:package 
  ...
  -->
	...
	<ctl:test name="reset:ATS_14.1.1.4_GET_Capabilities_Contents">
    ...
    <ctl:param name="cc.messagedir">INPUT MESSAGES</ctl:param>    
		<ctl:assertion>To verify that he RESET Server under test correctly returns the Contents section of a Capabilities resource.</ctl:assertion>
		<ctl:link title="14.1.1.4 GET Capabilities/Contents">http://www.opengis.net/spec/RESET/1.0/conf/Core/GET_Capabilities/Contents</ctl:link>
    <ctl:code>
     <xsl:variable name="contents.response">
        <ctl:request>
          <ctl:url>
               <xsl:copy-of select="concat($sp.endpoint.HTTP.url, 'Contents')"/>
          </ctl:url>
     ...
     <xsl:if test="not ($contents.response/*)">
          <ctl:message>ATS 14.1.1.4 validation failed</ctl:message>
          <ctl:fail/>
     </xsl:if>   	
    </ctl:code>
	</ctl:test>
