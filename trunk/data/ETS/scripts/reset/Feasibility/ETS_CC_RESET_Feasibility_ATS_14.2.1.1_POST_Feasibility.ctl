...
<ctl:package 
 ...
 xsi:schemaLocation="http://www.occamlab.com/ctl  ../apps/engine/resources/com/occamlab/te/schemas/ctl.xsd">
  ...
  -->
	...
	<ctl:test name="reset:ATS_14.2.1.1_POST_Feasibility">
		...
		<ctl:assertion>To verify whether the RESET Server under test is able to manage HTTP POST requests on Feasibility resource (i.e. it is able to create Feasibility resources).</ctl:assertion>
		<ctl:link title="14.2.1.1 POST Feasibility">http://www.opengis.net/spec/RESET/1.0/conf/Core/POST_Feasibility</ctl:link>
		<ctl:code>
	  ...
	  Verify that the RESET Server under test accepts the following request:
		...
		o	HTTP Entity Body:
		o	complies with the GetFeasibilityResponse described in the EO-SPS specification [NR22];
	  ...
	  #############################################################################################################-->
			...
			<!--############################################################################################################
	  ## Verifying response :																						##
	  ## - Is reset:Feasibility																						##
	  ## - Contains eosps:StatusReport																				##
	  ## - Contains eosps:FeasibilityStudy extension																##
	  ...
	  #############################################################################################################-->
			...
			<xsl:if test="not($response/reset:Feasibility)">
				<ctl:message>the feasibility submission failed</ctl:message>
				<ctl:fail/>
			</xsl:if>

			<!-- Status -->
			<xsl:variable name="resp.status" select="$response//reset:Feasibility//reset:status//reset:status//sps:taskStatus"/>
			<xsl:variable name="resp.procedure" select="$response//reset:Feasibility//reset:status//reset:status//sps:procedure"/>
			<xsl:variable name="resp.task" select="$response//reset:Feasibility//reset:status//reset:status//sps:task"/>
