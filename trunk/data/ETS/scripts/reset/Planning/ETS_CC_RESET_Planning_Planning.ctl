...
<ctl:package 
 ...
 -->
	...
	<ctl:test name="reset:RESET_Planning_Planning_Tests">
		...
		<!-- The following test has been removed because the TEAM engine is unable to validate the XML files sent back by the test server
		For all the different ways to create the said response
		
		  cvc-elt.2: The value of {abstract} in the element declaration for 'ows:AbstractReferenceBase' must be false.
