...
<ctl:package 
 ...
 <!-- CTL FILE METADATA
  ...
  Lineage: 2014/05/01 - ASTRIUM
	-->
	<ctl:test name="reset:ATS_14.3.1.3_GET_Task">
		...
		<ctl:link title="14.3.1.3 Get Task">http://www.opengis.net/spec/RESET/1.0/conf/Planning/GET_Planning_Task/</ctl:link>
		<ctl:code>
			<!-- #############################################################################################  -->
			<!-- #
        Verify that the RESET Server under test accepts the following request:
			...
			path>/RESET/1.0.0/planning/{taskID}/segments
		Verify the response message:
			...
			<xsl:variable name="endpoint.description" select="concat($sp.endpoint.HTTP.url,'planning/', $planning.task, '/segments')"/>
