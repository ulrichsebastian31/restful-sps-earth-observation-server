...
<ctl:package xmlns:ctl="http://www.occamlab.com/ctl" 
  ...
  -->
	...
	<ctl:test name="reset:ATS_14.3.1.4_POST_Planning_non_nominal_conditions">
		...
		<ctl:assertion>To verify whether the RESET Server under test is able to handle incorrect HTTP POST request to Planning resource.</ctl:assertion>
		<ctl:link title="11.3.4 Post Planning">http://www.opengis.net/spec/RESET/1.0/conf/Planning/POST_Planning-non-nominal</ctl:link>
    <ctl:code>
      
     <!-- #############################################################################################  -->
     <!-- #
        Send incorrect POST requests on Planning resource:
        •	HTTP Method: POST
        •	PATH: http://<hostname>:<port>/<context path>/RESET/1.0.0/planning
        Including several errors:
            1.	Planning element with a wrong tag e.g.: <reset:Planning>/<wrongTag>
            2.	Planning with with unsupported sensor: 
                a.	GetPlanning/procedure = WRONG_SENSOR
     ...
     <!-- #############################################################################################  -->
			...
			<xsl:if test="not ($response/*)">
				<ctl:message>ATS 14.3.1.4 validation failed</ctl:message>
				<ctl:fail/>
			</xsl:if>  
