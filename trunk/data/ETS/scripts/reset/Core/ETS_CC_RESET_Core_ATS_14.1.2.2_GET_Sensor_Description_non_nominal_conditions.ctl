...
<ctl:package 
 ...
 <!-- CTL FILE METADATA
  ...
  -->
	<ctl:test name="reset:ATS_14.1.2.2_GET_Sensor_Description_non_nominal_conditions">
		...
		<ctl:link title="14.1.2.2 Get Sensor Description/non_nominal_conditions">
		http://www.opengis.net/spec/RESET/1.0.0/conf/Core/GET_Sensor_Description-non-nominal
		</ctl:link>
		<ctl:code>
			<!-- #############################################################################################  -->
			<!-- #
          ...
          Pass if the assertion is satisfied; fail otherwise.
			...
			<xsl:variable name="endpoint.description" select="concat($sp.endpoint.HTTP.url, 'procedures/', 'NON_VALID_SENSOR', '/sensorML')"/>
