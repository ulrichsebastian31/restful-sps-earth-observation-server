<ctl:package 
  ...
  -->
	...
	<ctl:test name="reset:ATS_14.1.1.3_GET_Capabilities_ServiceProvider">
    ...
    <ctl:param name="cc.messagedir">INPUT MESSAGES</ctl:param>    
		<ctl:assertion>To verify that he RESET Server under test correctly returns the ServiceProvider section of a Capabilities resource.</ctl:assertion>
		<ctl:link title="14.1.1.3 GET Capabilities/ServiceProvider">http://www.opengis.net/spec/RESET/1.0/conf/Core/GET_Capabilities/ServiceProvider</ctl:link>
    <ctl:code>
     <xsl:variable name="provider.response">
        <ctl:request>
          <ctl:url>
               <xsl:copy-of select="concat($sp.endpoint.HTTP.url, 'ServiceProvider')"/>
          </ctl:url>
     ...
     <xsl:if test="not ($provider.response/*)">
          <ctl:message>ATS 14.1.1.3 validation failed</ctl:message>
          <ctl:fail/>
     </xsl:if>   	
    </ctl:code>
	</ctl:test>
