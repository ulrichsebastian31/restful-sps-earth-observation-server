...
public class DescribeResultAccessOperation extends EOSPSOperation<MissionPlannerDBHandler,DescribeResultAccessDocument,DescribeResultAccessResponseDocument,DescribeResultAccessFault> {
	...
	public void executeRequest() throws DescribeResultAccessFault{
        ...
        //                    DataAvailableType.DataReference ref = dataAvailable.addNewDataReference();
                            DataAvailableType.DataReference ref = DataAvailableType.DataReference.Factory.newInstance(OGCNamespacesXmlOptions.getInstance());
                            ...
                            for (Product segmentProducts : products.get(segmentId)) {
                                //Product
                                ReferenceDocument refDoc = ReferenceDocument.Factory.newInstance(OGCNamespacesXmlOptions.getInstance());
                                ReferenceType reff = refDoc.addNewReference();
                                
                                AbstractReferenceBaseType refBase = grp.addNewAbstractReferenceBase();
                                
                                reff.setHref(segmentProducts.getDownloadURL());        //Product URL
                                reff.setRole("PRODUCT_" + segmentProducts.getProductID());          //Product ID
                                reff.setArcrole("SEGMENT_" + segmentProducts.getMmfasSegmentID());  //Segment ID
